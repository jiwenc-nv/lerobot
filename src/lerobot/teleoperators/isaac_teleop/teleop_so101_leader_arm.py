...
# Gripper closedness [0, 1] -> motor units [0, 100] (RANGE_0_100). The SO-101 follower
# calibrates 100 = fully OPEN, 0 = fully CLOSED, so the closedness is inverted here
# (``pos = (1 - c) * 100``), matching ``xr_controller_processor._GRIPPER_MOTOR_SCALE``.
...
class SO101LeaderArm(IsaacTeleopTeleoperator):
    ...
    def _running_events(self):
        """Constant ``RUNNING`` ``ExecutionEvents`` for this frame.

        There is no clutch/retargeter lifecycle to gate, so the session is always driven
        ``RUNNING`` to keep the joint stream flowing (the same rationale as
        :meth:`XRController._running_events`); ``reset`` is left ``False``.
        """
