...
startup uses. One episode is one uninterrupted squeeze, so the mid-task re-clutch described above is
a teleoperation move, not a recording one. `--reset_to_origin=false` disables the slew (startup and
between episodes alike), and the leader arm, having no clutch, still ends episodes on
`--dataset.episode_time_s` or a key.
