...
bindings. `retargeters-lite` is the scipy-only retargeter tier: this example runs no Isaac
retargeter at all — both devices expose their source nodes directly and IK is LeRobot's Placo
pipeline — so the full `retargeters` extra (`dex-retargeting`, `nlopt`, `torch`) would add
several packages nothing here imports. Install it alongside if you want the hand retargeters.
