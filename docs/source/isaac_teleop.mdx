...
example runs is the SO-101 clutch inside `XRController` (the leader arm exposes its source node
directly, and IK is LeRobot's Placo pipeline), so the full `retargeters` extra
...
**The clutch is the episode boundary.** On the XR path an episode ends the moment you release the
...
and the leader arm, having no clutch, still ends episodes on `--dataset.episode_time_s` or a key.
